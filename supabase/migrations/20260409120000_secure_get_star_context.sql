-- =============================================================================
-- secure_get_star_context
-- Replaces public.get_star_context(uuid) with auth + membership + policy wrapper.
--
-- Aggregate body: live SELECT jsonb_build_object(...) INTO v_result from staging
-- `pg_get_functiondef`, with studio_director / teacher CASE predicates per
-- app/docs/operator/staging-get-star-context/ROLE_HANDLING.md (parent/student still blocked below).
--
-- service_role: explicitly REVOKED here. If a job must call this RPC, add:
--   GRANT EXECUTE ON FUNCTION public.get_star_context(uuid) TO service_role;
-- in a follow-up migration with a comment.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- Policy helper — post-process + array trims (defense in depth).
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.star_apply_star_context_policy(
  p_payload jsonb,
  p_role text,
...
DECLARE
  ...
  v_mrr_sum bigint;
BEGIN
  IF p_allowed_location_ids IS NOT NULL AND cardinality(p_allowed_location_ids) > 0 THEN
    SELECT COALESCE(array_agg(DISTINCT e->>'name' ORDER BY e->>'name'), ARRAY[]::text[])
    INTO v_names
    FROM jsonb_array_elements(COALESCE(v->'locations', '[]'::jsonb)) e
    WHERE (e->>'id')::uuid = ANY(p_allowed_location_ids);

    SELECT COALESCE(jsonb_agg(e ORDER BY e->>'name'), '[]'::jsonb)
    INTO v_loc
    FROM jsonb_array_elements(COALESCE(v->'locations', '[]'::jsonb)) e
    WHERE (e->>'id')::uuid = ANY(p_allowed_location_ids);

    v := jsonb_set(v, '{locations}', v_loc, true);

    IF v ? 'students' THEN
      v := jsonb_set(v, '{students,by_location}',
        COALESCE((
          SELECT jsonb_agg(e ORDER BY (e->>'count')::int DESC)
          FROM jsonb_array_elements(COALESCE(v->'students'->'by_location', '[]'::jsonb)) e
          WHERE e->>'location' = ANY(v_names)
        ), '[]'::jsonb), true);
    END IF;

    IF v ? 'schedule' THEN
      v := jsonb_set(v, '{schedule,by_location_this_week}',
        COALESCE((
          SELECT jsonb_agg(e ORDER BY (e->>'booked')::int DESC)
          FROM jsonb_array_elements(COALESCE(v->'schedule'->'by_location_this_week', '[]'::jsonb)) e
          WHERE e->>'location' = ANY(v_names)
        ), '[]'::jsonb), true);
    END IF;

    IF v ? 'billing' THEN
      v := jsonb_set(v, '{billing,mrr_by_location}',
        COALESCE((
          SELECT jsonb_agg(e ORDER BY (e->>'mrr_cents')::bigint DESC)
          FROM jsonb_array_elements(COALESCE(v->'billing'->'mrr_by_location', '[]'::jsonb)) e
          WHERE e->>'location' = ANY(v_names)
        ), '[]'::jsonb), true);

      SELECT COALESCE(SUM((e->>'mrr_cents')::bigint), 0)
      INTO v_mrr_sum
      FROM jsonb_array_elements(COALESCE(v->'billing'->'mrr_by_location', '[]'::jsonb)) e;

      v := jsonb_set(v, '{billing,estimated_mrr_cents}', to_jsonb(v_mrr_sum), true);
    END IF;

    IF v ? 'teachers' AND v->'teachers' ? 'by_location' THEN
      v := jsonb_set(v, '{teachers,by_location}',
        COALESCE((
          SELECT jsonb_agg(e ORDER BY (e->>'teacher_count')::int DESC)
          FROM jsonb_array_elements(COALESCE(v->'teachers'->'by_location', '[]'::jsonb)) e
          WHERE e->>'location' = ANY(v_names)
        ), '[]'::jsonb), true);
    END IF;

    IF v ? 'leads' AND v->'leads' ? 'by_location' THEN
      v := jsonb_set(v, '{leads,by_location}',
        COALESCE((
          SELECT jsonb_agg(e)
          FROM jsonb_array_elements(COALESCE(v->'leads'->'by_location', '[]'::jsonb)) e
          WHERE e->>'location' = ANY(v_names)
        ), '[]'::jsonb), true);
    END IF;

    IF v ? 'sessions' AND v->'sessions' ? 'by_location_last_30_days' THEN
      v := jsonb_set(v, '{sessions,by_location_last_30_days}',
        COALESCE((
          SELECT jsonb_agg(e)
          FROM jsonb_array_elements(COALESCE(v->'sessions'->'by_location_last_30_days', '[]'::jsonb)) e
          WHERE e->>'location' = ANY(v_names)
        ), '[]'::jsonb), true);
    END IF;
  END IF;
  ...
  IF p_role = 'teacher' THEN
    v := jsonb_set(v, '{billing}', jsonb_build_object(
      'estimated_mrr_cents', 0,
      'mrr_by_location', '[]'::jsonb,
      'rate_tiers', COALESCE(v->'billing'->'rate_tiers', '[]'::jsonb),
      'latest_billing_cycle', COALESCE(v->'billing'->'latest_billing_cycle', 'null'::jsonb)
    ), true);

    v := jsonb_set(v, '{families}', (COALESCE(v->'families', '{}'::jsonb) || jsonb_build_object(
      'total_overdue_cents', 0,
      'families_overdue', 0
    )), true);

    v := jsonb_set(v, '{payroll}', jsonb_build_object('latest_period', null), true);

    v := jsonb_set(v, '{leads}', jsonb_build_object(
      'active_total', 0,
      'needing_followup', 0,
      'new_last_7_days', 0,
      'new_last_30_days', 0,
    ...
    v := jsonb_set(v, '{tasks}', jsonb_build_object(
      'open', 0, 'overdue', 0, 'high_priority_open', 0
    ), true);

    v := jsonb_set(v, '{communications}', jsonb_build_object(
      'sent_last_7_days', 0,
      'sent_last_30_days', 0
    ), true);
  END IF;
  ...
  IF p_role = 'studio_director' THEN
    v := jsonb_set(v, '{payroll}', jsonb_build_object('latest_period', null), true);
    v := jsonb_set(v, '{billing}', (COALESCE(v->'billing', '{}'::jsonb) || jsonb_build_object(
      'rate_tiers', '[]'::jsonb,
      'latest_billing_cycle', 'null'::jsonb
    )), true);
  END IF;
...
REVOKE ALL ON FUNCTION public.star_apply_star_context_policy(jsonb, text, uuid[]) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.star_apply_star_context_policy(jsonb, text, uuid[]) FROM anon;

-- -----------------------------------------------------------------------------
-- Main RPC
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.get_star_context(p_tenant_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DECLARE
  ...
  v_teacher_row public.teachers%ROWTYPE;
  v_result jsonb;
BEGIN
  IF v_uid IS NULL THEN
    RAISE EXCEPTION 'get_star_context: authentication required'
      USING ERRCODE = '28000';
  END IF;
  ...
  IF v_profile_id IS NULL THEN
    RAISE EXCEPTION 'get_star_context: tenant membership denied'
      USING ERRCODE = '42501';
  END IF;

  IF v_role IS NULL OR v_role = '' THEN
    RAISE EXCEPTION 'get_star_context: profile role is null'
      USING ERRCODE = '42501';
  END IF;

  -- Explicit blocks (see app/docs/operator/staging-get-star-context/ROLE_HANDLING.md)
  IF v_role = 'student' THEN
    RAISE EXCEPTION 'get_star_context: role student is not authorized for this RPC'
      USING ERRCODE = '42501';
  END IF;

  IF v_role = 'parent' THEN
    RAISE EXCEPTION 'get_star_context: parent scope not implemented — splice aggregate and remove this raise'
      USING ERRCODE = '0A000',
      HINT = 'Implement linkage-limited aggregate, then delete this IF block.';
  END IF;

  IF v_role NOT IN ('owner', 'admin', 'studio_director', 'teacher') THEN
    RAISE EXCEPTION 'get_star_context: unknown or disallowed role %', v_role
      USING ERRCODE = '42501';
  END IF;
  ...
  IF v_role = 'studio_director' THEN
    ...
    IF cardinality(v_allowed_location_ids) = 0 THEN
      RAISE EXCEPTION 'get_star_context: studio_director has no profile_locations'
        USING ERRCODE = '42501';
    END IF;
  ELSE
  ...
  IF v_role = 'teacher' THEN
    ...
    FROM teachers t
    WHERE t.profile_id = v_profile_id AND t.tenant_id = p_tenant_id AND t.is_active = true
    LIMIT 1;
  END IF;
  ...
  v_result := public.star_apply_star_context_policy(v_result, v_role, v_allowed_location_ids);

  IF v_role = 'teacher' AND v_teacher_row.id IS NOT NULL THEN
    v_result := jsonb_set(v_result, '{teachers,load_by_teacher}',
      COALESCE((
        SELECT jsonb_agg(e)
        FROM jsonb_array_elements(COALESCE(v_result->'teachers'->'load_by_teacher', '[]'::jsonb)) e
        WHERE e->>'name' = trim(both from (v_teacher_row.first_name || ' ' || v_teacher_row.last_name))
      ), '[]'::jsonb), true);
  END IF;
...
-- -----------------------------------------------------------------------------
-- Grants — explicit (no accidental service_role access)
-- -----------------------------------------------------------------------------
REVOKE ALL ON FUNCTION public.get_star_context(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_star_context(uuid) FROM anon;
REVOKE EXECUTE ON FUNCTION public.get_star_context(uuid) FROM service_role;

GRANT EXECUTE ON FUNCTION public.get_star_context(uuid) TO authenticated;

-- If automation must call this RPC with the service role JWT, add in a small follow-up migration:
-- GRANT EXECUTE ON FUNCTION public.get_star_context(uuid) TO service_role;
